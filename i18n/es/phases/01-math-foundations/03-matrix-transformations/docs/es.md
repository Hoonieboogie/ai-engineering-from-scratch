...
![Rotación como matriz (2D): las columnas de R(θ) = [[cos θ, -sin θ], [sin θ, cos θ]] son donde los vectores de base aterrizan después de una rotación en sentido contrario al reloj por θ. Para θ = 90°, [1,0] → [0,1] y [0,1] → [-1,0]; las longitudes y ángulos no cambian.](matrix_rotation.png)
...
![Ciecer (2D): una correa horizontal Shx = [[1, k], [0, 1]] se desliza cada punto hacia un lado en proporción a su altura (k=1 envía [0,1] → [1,1]), mientras que una correa vertical Shy = [[1, 0], [k, 1]] se apunta hacia arriba en proporción a su x. Los rectángulos se convierten en paralelogramas; las áreas permanecen iguales.](matrix_shearing.png)
