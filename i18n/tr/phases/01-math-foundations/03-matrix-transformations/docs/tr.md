...
![Matrix olarak dönüm (2D): R(θ) = [[cos θ, -sin θ], [sin θ, cos θ]] sütunları, temel vektörlerin saat yönünde dönüp gittikten sonra θ'ye düştüğü yerdir. θ = 90° için, [1,0] → [0,1] ve [0,1] → [-1,0]; uzunluklar ve açılar değişmez.](matrix_rotation.png)
...
![Kürümüş (2D): Dörtgen bir kesim Shx = [[1, k], [0, 1]] her noktayı yüksekliğine göre yan çizer (k=1 gönderir [0,1] → [1,1]), dikey bir kesim Şay = [[1, 0], [k, 1]] çizimler x'lerine göre yukarı doğru gösterir. Dörtgenler paralelogram olur; alanlar aynı kalır.](matrix_shearing.png)
