...
![转换为矩阵 (2D):R(θ) = [[cos θ, -sin θ],[sin θ, cos θ]]的列是基向量在以 θ为反钟方向转换后降落的地方.对于 θ = 90°, [1,0] → [0,1]和 [0,1] → [-1,0];长度和角形没有变化.](matrix_rotation.png)
...
![切割 (2D):一个水平切割 Shx = [[1, k], [0, 1]] 每个点按其高度的比例向侧滑动 (k=1发送 [0,1] → [1,1]),而一个垂直切割 Shy = [[1, 0], [k, 1]] 切片以 x 的比例向上.矩形变为平行ogram;区域保持相同.](matrix_shearing.png)
