...
![التناوب كصفوفة (2D): أعمدة R(θ) = [[cos θ، -sin θ] ، [sin θ، cos θ]] حيث يتمكن المتجهات الأساسية من الهبوط بعد دوران في اتجاه المعاكس مع θ. بالنسبة إلى θ = 90 °، [1,0] → [0,1] و [0,1] → [-1,0]; لا تتغير الطول والزوايا.](matrix_rotation.png)
...
![تقطيع (2D): تقطيع أفقي Shx = [[1، k] ، [0, 1]] ينزلق كل نقطة جانباً متناسبة مع ارتفاعها (k=1 يرسلها [0,1] → [1,1]) ، في حين أن التقطيع عمودي Shy = [[1, 0] ، [k، 1]] يسلط إلى الأعلى متناسبة مع x. يصبح المثلثات متوازية؛ المناطق تظل نفسها.](المصفوفات_تقطيع.png)
