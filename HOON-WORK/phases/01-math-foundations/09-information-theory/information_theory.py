...
def information_content(p, base=2):
    ...
    if p >= 1:
        return 0.0
    return -math.log(p) / math.log(base)  # log_b(p) = ln(p) / ln(b)
...
def negative_log_likelihood(labels, all_logits):
    return sum(
        cross_entropy_loss(label, logits) for label, logits in zip(labels, all_logits)
    ) / len(labels)
...
def feature_selection_mi_demo():
    ...
    features["strong_signal"] = [
        t ^ (1 if random.random() < 0.1 else 0) for t in target
    ]
...
if __name__ == "__main__":
    ...
    print(
        f"  Good: {h_true:.4f} + {kl_good:.4f} = {h_true + kl_good:.4f}  (CE = {ce_good:.4f})"
    )
    print(
        f"  Bad:  {h_true:.4f} + {kl_bad:.4f} = {h_true + kl_bad:.4f}  (CE = {ce_bad:.4f})"
    )
    ...
    all_logits = [
        [random.gauss(0, 1) for _ in range(n_classes)] for _ in range(n_samples)
    ]

    ce_avg = negative_log_likelihood(labels, all_logits)
    nll_avg = (
        -sum(math.log(softmax(lg)[lb]) for lb, lg in zip(labels, all_logits))
        / n_samples
    )
    ...
    token_logits = [
        [random.gauss(0, 1) for _ in range(vocab_size)] for _ in range(sequence_length)
    ]
    ...
    print(
        f"    Verify: H(X,Y) = H(X) + H(Y|X) = {hx_dep:.4f} + {conditional_entropy(joint_dep):.4f} = {hx_dep + conditional_entropy(joint_dep):.4f}"
    )
